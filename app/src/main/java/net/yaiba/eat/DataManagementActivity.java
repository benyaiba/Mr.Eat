...
public class DataManagementActivity extends Activity {
	...
	//private String fileName = "eatres.xml";
	//private String fileNamePre = "eat_";
	private String fileNameSuff = ".xml";
	private String fileNameMass = "eatres_mass.xml";
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		String eatPath = Environment.getExternalStorageDirectory().toString()  + "//" +FILE_DIR_NAME;
		File[] files = new File(eatPath).listFiles();

		setContentView(R.layout.activity_data_management);
		if(files != null){
			for (int i = 0; i < files.length; i++) {
				File file = files[i];
				if(file.getName().indexOf("!!!!!") != -1 || file.getName().indexOf("eatres_mass")!= -1){
 	    ...
 	    try {
 	       ...
 	       if(mCursor.moveToFirst()) {
		        while(!mCursor.isAfterLast()) {
					...
					//加密场合原样输出，未加密场合解密处理
					serializer.attribute("", "password", encode?mCursor.getString(3):DESUtils.decryptDES(mCursor.getString(3)));
		                         ...
		                         if (xrp.getEventType() == XmlPullParser.START_TAG) {
		                              ...
		                              if(tagName.equals("record")){   
		                                  ...
		                                  if(recoverType.equals("mass")){
		                                	  password = DESUtils.encryptDES(password);
		              					   }
		                                  if(!site.isEmpty() && !user.isEmpty() && !password.isEmpty() ){
		                                	  if(remark.equals(null)){
		                                		  remark="";
		                                	  }
											  FoodDB.insert(site, user);
		                                  }
		                              }   
