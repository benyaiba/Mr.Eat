...
public class LoginPEditActivity extends Activity {
	...
	protected Boolean change(){
		...
		try {
			if(LoginDB.isCurrentPassword(DESUtils.encryptDES(oldPassword)) >= 0){
				LoginDB.update(LoginDB.isCurrentPassword(DESUtils.encryptDES(oldPassword)), DESUtils.encryptDES(password));
				return true;
			}else{
